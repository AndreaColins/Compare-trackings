function varargout = comparetracking2(varargin)
...
% Last Modified by GUIDE v2.5 24-Oct-2016 15:23:03
...
%             r3all(handles.currentframe,:,:) = r3new;
        ...
        % 080116: I've not yet attempted to generalise the following
        % conditional:
        if get(handles.radiobutton_kinematics_test,'Value')
            ...
            rad = abs(1/kappa);
            ttest = 0.5;
            cc = bezierval(squeeze(rall(handles.currentframe.h,:,:)),ttest);
            tmp = bezierval(squeeze(rall(handles.currentframe.h,:,:)),ttest+[-.01 .01]);
...
%      
    ...
    if isfield(handles,'comparetr4')
    load(handles.comparetr4,'whisker','calib','-mat')
    
    for w = 1:length(whisker)
        
     wmod = handles.ncompare; % to avoid running out of colours
        if any(whisker(w).tracked)    
            r3 = squeeze(whisker(w).r3all(handles.currentframe.h,:,:));
            b3 = bezierval(r3,t);
            r2 = projection2(r3,handles.calib);
            b2 = projection2(b3,handles.calib);
            axes(handles.viewh), hold on
...
titles.h = ''; titles.v = '';
% plot_currentframe(handles,titles)
plot_frame(frame,handles.video,handles.meanframe,titles,handles)
...
if isfield(handles,'comparetr4')
file2=load(handles.comparetr4,'-mat');
end
...
if exist('trpmtrs','var')
    ...
    %%%%%in the original comparetracking2.m says
...
haxes.h = handles.viewh;
haxes.v = handles.viewv;
plot_frame(handles.frame,handles.video,handles.meanframe,titles,haxes)
for w = 1:handles.Nwhiskers
    wmod = rem(w,handles.Nwhiskers) + handles.Nwhiskers*(w==handles.Nwhiskers); % to avoid running out of colours
    if handles.whisker(w).selected && handles.whisker(w).tracked(frameidx)
        dt = 0.05;
...
contents = cellstr(get(hObject,'String'));
handles.comparetr4 = contents{get(hObject,'Value')};
clear contents
load(handles.comparetr4,'whisker','roi','calib','trpmtrs','-mat')
