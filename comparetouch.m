function [idx,result]=comparetouch(files,nm)
...
meandata=mean(result(1:500,:,f));
result(:,1,f)=result(:,1,f)-meandata(1);
result(:,2,f)=result(:,2,f)-meandata(2);
result(:,3,f)=result(:,3,f)-meandata(3);
result(:,4,f)=result(:,4,f)-meandata(4);
limith1=0.001+meandata(4);
limith2=(-0.001-meandata(4));
I=find(result(:,4,f)>limith1 & result(:,4,f)<limith2);
result(I,:,f)=0;
plot(idx,result(:,4,f),colours{wmod})
end
idx=idx';
clear w coronal horizontal hor
